...
shoppers-own [ shopping-list not-yet-visited spent ]
...
to setup
  ...
  foreach xs [
    create-traders 1 [
      ...
      foreach stock [
        set prices lput ((1 + mark-up) * (item (position ? fruit-and-veg) fruit-and-veg-prices)) prices
      ]
    ]
  ]
...
to go
  ...
  ask shoppers [
    ...
    foreach route [
      ...
      foreach purchases [
        set spent spent + produce-price ? stall 
        ; delete the items that have been bought from the shopping list    
        set shopping-list remove ? shopping-list
      ] 
  ...
  repeat n-scans [
    ...
    while [ not empty? to-buy] [
      ...
      if not empty? purchases [
        ...
        foreach purchases [
          set cost cost + produce-price ? stall
          ; delete the items that have been bought    
          set to-buy remove ? to-buy
        ]
      ]
    ] 
...
to-report buy-from-stall [ what-to-buy stall ]
  report filter [ member? ? [stock] of stall] what-to-buy
end
